...
## Milestone 6 Notes
*Notes taken after finishing Milestone 6 (Flow Control, Expressions, Action Blocks).*

- **Action tree infrastructure:** `ActionNode` data class + `buildActionTree()`/`flattenActionTree()` in `:core:model`. Engine's `ActionExecutor` refactored to walk the tree recursively instead of a flat list. `FlowResult` sealed class (Continue, Break, ContinueLoop, CancelMacro) for non-exception control flow.
- **FlowControlHandler interface** extends `ActionHandler` with an `executeBlock` callback parameter. The executor checks handler type at dispatch time and passes child nodes + a block-execution lambda to flow control handlers.
- **Two-pass DB save** in `MacroRepository.saveMacroWithDetails()` for nested actions: inserts all actions with `parentActionId = null` first to get real DB IDs, then updates parent references using a temp-ID-to-real-ID mapping. Editor uses negative temp IDs internally.
- **6 flow control handlers:** IfClause (with ElseMarker child splitting), Repeat (count/while modes), Break, Continue, CancelMacro, plus ElseMarker (no-op separator).
- **Expression engine:** Custom recursive-descent parser in `ExpressionEngine.kt` (~400 lines). Supports arithmetic (`+`,`-`,`*`,`/`,`%`), comparison (`==`,`!=`,`>`,`<`,`>=`,`<=`), boolean (`&&`,`||`,`!`), string functions (`contains()`,`startsWith()`,`endsWith()`,`length()`,`matches()`), parentheses, literals, and variable references (`v_name`, `lv_name`). `ExpressionValue` sealed class with `NumberVal`, `StringVal`, `BoolVal` and type coercion.
- **Advanced variable types:** `ARRAY` and `DICTIONARY` added to `VariableType`. Stored as JSON in existing `valueJson` column. No DB migration needed.
- **5 new action handlers:** Iterate (FlowControlHandler for arrays/dicts, sets `lv_key`/`lv_value`), ArrayManipulation (push/pop/insert/remove/sort/reverse/clear/size), JsonParse (dot-path extraction), TextManipulation (substring/replace/split/join/trim/case/regex), WaitUntilTrigger (pauses coroutine via `suspendCancellableCoroutine` until a trigger fires or timeout).
- **Action Blocks (reusable functions):** New `action_blocks` table (DB migration 2→3). `ActionBlock` domain model with input/output parameter definitions. `RunActionBlockHandler` creates a child `ExecutionContext` with fresh `localVariables`, copies input params in, runs block's actions, copies output params back. New `:feature:actionblocks` module with list + editor screens.
- **Editor UI supports nesting:** Flattened display list with depth metadata (not nested LazyColumns). Each nesting level gets 24dp left padding + colored left border. Flow control actions show "Add Action" button inside their block. ElseMarker renders as a divider. Collapse/expand toggle on flow control actions.
- **MagicTextField wrapper:** Replaces `OutlinedTextField` in action config editors. Adds a `{}` icon button that opens `MagicTextPickerSheet`. Uses `CompositionLocal` (`LocalUserVariables`, `LocalTriggerTypeIds`) to pass context to deeply nested composables without threading params through every function.
- **MagicTextPickerSheet** shows trigger-specific tokens first (filtered by macro's trigger types), then user-defined variables from `VariableStore`, then built-in tokens. Critical for discoverability.
- **MagicTextResolver regex fix:** Changed from `\{([^}]+)\}` to `\{(\w+)\}`. The old regex matched JSON structural braces — e.g., in `{"body":"{sms_message}"}` it would match from the JSON opening `{` through to `}` after `sms_message`, capturing `"body":"{sms_message` as the token. The `\w+` pattern only matches word characters, preventing this.
- **SMS Sent trigger added:** `SmsSentMonitor` uses `ContentObserver` on `Telephony.Sms.CONTENT_URI` to detect outgoing messages. Queries `Telephony.Sms.Sent.CONTENT_URI` for new rows. Requires `READ_SMS` runtime permission (same permission group as `RECEIVE_SMS`). Emits `{sms_recipient}` and `{sms_message}` in trigger data.
- **New magic text tokens:** `{sms_recipient}` for SMS Sent trigger data. `{lv_repeat_index}`, `{lv_key}`, `{lv_value}` for flow control iteration context.
- **`:feature:actionblocks` depends on `:feature:macros`** to reuse action config editors, `TypePickerSheet`, and shared composables (`ActionConfigContent`, `SectionHeader`, `AddButton`, `AVAILABLE_ACTIONS`, `actionIcon`). One-way dependency.
